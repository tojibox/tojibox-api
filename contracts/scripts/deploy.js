/**
 * Deploy TojiboxReportReceipt to GIWA Sepolia.
 *
 * Usage:
 ...
 *
 * After deploy, copy CONTRACT_ADDRESS into tojibox-api/.env as
 * TOJIBOX_REPORT_RECEIPT_ADDRESS
 */
...
async function main() {
  ...
  console.log("Deploying TojiboxReportReceipt...");
  ...
  const TojiboxReportReceipt = await hre.ethers.getContractFactory("TojiboxReportReceipt");
  ...
  const contract = await TojiboxReportReceipt.deploy(oracleAddress);
  await contract.waitForDeployment();

  const address = await contract.getAddress();

  console.log("\nTojiboxReportReceipt deployed!");
  console.log("  Contract address :", address);
  console.log("  Oracle address   :", oracleAddress);
  console.log("\nAdd to tojibox-api/.env:");
  console.log(`  TOJIBOX_REPORT_RECEIPT_ADDRESS=${address}`);
}
