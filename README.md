...
FastAPI oracle serving layer for **Togibox** — a Wake County / Raleigh NC
...
- Exposes `/api/oracle/*` REST endpoints consumed by the `togibox-app`
  frontend and the `togibox-scraper` pipeline/committer
- Serves parcel details, rezoning petition history, and change-event logs
  from Postgres (shared DB with `togibox-scraper`)
...
- Signs each generated report with the oracle's ECDSA key (EIP-191 /
  `personal_sign`) and mints a `TogiboxReportReceipt` NFT on GIWA recording
...
- **Solidity 0.8.20 / Hardhat / OpenZeppelin** — `TogiboxReportReceipt.sol`
  (ERC-721)
...
│   └── abi/TogiboxReportReceipt.json
├── contracts/
│   ├── src/TogiboxReportReceipt.sol   # ERC-721 report receipt contract
...
cp .env.example .env       # fill in DB_*, GIWA_ORACLE_PRIVATE_KEY, TOGIBOX_REPORT_RECEIPT_ADDRESS, ...
...
# copy the printed contract address into ../.env as TOGIBOX_REPORT_RECEIPT_ADDRESS
```
...
| `DB_HOST` / `DB_PORT` / `DB_USER` / `DB_PASSWORD` / `DB_NAME` | Postgres connection (shared with togibox-scraper) |
...
| `TOGIBOX_REPORT_RECEIPT_ADDRESS` | Deployed `TogiboxReportReceipt` contract address |
...
- **Report verification is no longer restart-fragile**: unlike the
  ...
  back to reading the `TogiboxReportReceipt` contract on-chain, so a report
